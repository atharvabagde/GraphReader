...
class GraphReader:
    def __init__(self,graph, vect_db_name = 'graph-reader',  **kwargs):
        self.vect_db = vect_db_name
        self.graph = graph
        self.pine_client = Pinecone_client()
        self.pine_client.upsert_data(self.vect_db,self.graph)
        self.llm = kwargs.get('llm_model',ChatOpenAI(model="gpt-3.5-turbo",api_key=os.environ['OPENAI_API_KEY']))
        self._load_prompts()
        self._load_json_struct()
        
    def _load_prompts(self,prompts_file_path = 'prompts.yaml'):
