...
class GraphReader:
    def __init__(self,
        ...
        assert self.pinecone_api_key is not None, "Pinecone API key must be provided either as an argument or through the environment variable 'PINECONE_API_KEY'."
        if not kwargs.get('llm_model'):
            assert self.openai_api_key is not None, "OpenAI API key must be provided either as an argument or through the environment variable 'OPENAI_API_KEY'."
    ...
    def _load_prompts(self,prompts_file_path = 'prompts/prompts.yaml'):
